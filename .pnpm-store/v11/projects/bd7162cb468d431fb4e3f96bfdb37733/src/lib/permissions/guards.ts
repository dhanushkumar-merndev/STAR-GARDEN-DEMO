...
/**
 * A lost or closed lead is retained for its audit history, but it must not
 * begin a new delivery phase. Existing visit/design/execution records remain
 * visible so an Admin can make an explicit cancellation decision.
 */
export function assertLeadCanStartDelivery(lead: LeadRow): void {
  if (lead.status === 'LOST' || lead.status === 'CLOSED') {
    throw new AppError(
      'INVALID_TRANSITION',
      `This lead is ${lead.status === 'LOST' ? 'not interested' : 'closed'}. Reopen it before starting new delivery work.`,
    );
  }
}
