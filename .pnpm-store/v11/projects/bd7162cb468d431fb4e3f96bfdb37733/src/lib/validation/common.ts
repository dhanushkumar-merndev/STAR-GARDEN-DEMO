...
/**
 * One-time, visit-scoped coordinates. Browser permission is still explicit;
 * the visit action fails if staff do not grant it.
 */
...
export const requiredLatitude = z.coerce.number().min(-90).max(90);
export const requiredLongitude = z.coerce.number().min(-180).max(180);
