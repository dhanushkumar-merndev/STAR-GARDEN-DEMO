  ...
  switch (outcome) {
    ...
    case 'INTERESTED':
      return current === 'ASSIGNED' || current === 'NEW' || current === 'UNASSIGNED'
        ...
        : null;
    case 'CONNECTED':
      // A completed callback returns the lead from the Follow-up queue to the
      // normal contacted stage. Later workflow stages must never be regressed.
      return current === 'ASSIGNED' || current === 'NEW' || current === 'UNASSIGNED' || current === 'FOLLOW_UP'
        ? 'CONTACTED'
        : null;
    case 'CALL_LATER':
    case 'NO_ANSWER':
