/**
 * Map primitives — vendored from mapcn (https://mapcn.dev), MapLibre GL under
 * the hood, with two changes for this codebase:
 *
 *   - icons come from react-icons rather than lucide-react, which the CRM no
 *     longer depends on;
 *   - the cn helper is imported from its actual home here, @/lib/utils/cn.
 *
 * Upstream is a shadcn-style registry, so this is source we own: re-fetching it
 * means re-applying those two edits.
 */
"use client";
...
>({
  ...
  useEffect(() => {
    ...
    const handleClusterClick = async (
      ...
      // Upstream indexes straight into [0]; this project compiles with
      // `noUncheckedIndexedAccess`, so the element is narrowed rather than
      // only the array length being checked.
      if (!feature) return;
    ...
    const handlePointClick = (
      ...
      const feature = e.features[0];
      if (!feature) return; // See the note in the cluster handler above.
