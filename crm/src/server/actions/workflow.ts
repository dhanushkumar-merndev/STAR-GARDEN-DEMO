...
import {
  ...
  assignVisitDesignerSchema,
  dispositionSchema_input,
  startJourneySchema,
...
import {
  assignVisitDesigner as assignVisitDesignerService,
  ...
  startJourney as startJourneyService,
} from '@/server/services/site-visits';
import {
  recordDisposition as recordDispositionService,
  type DispositionResult,
} from '@/server/services/disposition';
import {
  ...
  startDesignFromCompletedVisit as startDesignFromCompletedVisitService,
 ...
 */
/**
 * "Start" — the designer is on the way.
 *
 * Revalidates the dashboard too: the Admin's board shows who is travelling, and
...
export async function startJourneyAction(
  ...
  return actionResult(async () => {
    const user = await requireUser();
    const input = parseOrThrow(startJourneySchema, formDataToObject(formData));

    const visit = await startJourneyService(user, input);
...
/** Puts a different landscape designer on an already-booked visit. Admin-only. */
export async function assignVisitDesignerAction(
  _prev: unknown,
  formData: FormData,
): Promise<ActionResult<{ siteVisitId: string }>> {
  return actionResult(async () => {
    const user = await requireUser();
    const input = parseOrThrow(assignVisitDesignerSchema, formDataToObject(formData));

    const visit = await assignVisitDesignerService(user, input);
 ...
 *
 * Returns `nextStep` so the form can immediately open the right follow-on —
 * book a visit, assign a design — instead of leaving the user to work out what
 * "Interested" was supposed to lead to.
 */
export async function recordDispositionAction(
  _prev: unknown,
  formData: FormData,
): Promise<ActionResult<DispositionResult['nextStep'] & { leadId: string }>> {
  return actionResult(async () => {
    const user = await requireUser();
    const input = parseOrThrow(dispositionSchema_input, formDataToObject(formData));

    const result = await recordDispositionService(user, input);
    ...
    return { ...result.nextStep, leadId: result.lead.id };
  });
}
...
export async function startDesignFromVisitAction(
  _prev: unknown,
  formData: FormData,
): Promise<ActionResult<{ designProjectId: string }>> {
  return actionResult(async () => {
    const user = await requireUser();
    const input = parseOrThrow(startDesignFromVisitSchema, formDataToObject(formData));
    const project = await startDesignFromCompletedVisitService(user, input);
