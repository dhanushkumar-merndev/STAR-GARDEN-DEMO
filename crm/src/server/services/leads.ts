...
import type { LeadRow, LeadSource, LeadStatus } from '@/types/database';
...
export async function createManualLead(
  ...
  // A BDM creating their own lead owns it immediately — an unowned lead with no
  // next action is exactly what §8.2's dashboard flag exists to catch.
  if (!assignedTo && user.role === 'BDM') assignedTo = user.id;
...
export async function getLeadDetail(user: SessionUser, leadId: string) {
  ...
  const [activities, followUps, siteVisits, designProject, executionProject, owner, files] =
    await Promise.all([
      supabase
        .from('activities')
...
/** Active BDMs, for assignment pickers. */
