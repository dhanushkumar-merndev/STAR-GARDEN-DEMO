...
describe('design upload permissions', () => {
  it('allows only the assigned Landscape Designer to upload a design version', () => {
    expect(canUploadDesignVersion({ id: 'designer-1', role: 'LANDSCAPER' }, project)).toBe(true);
    expect(canUploadDesignVersion({ id: 'designer-2', role: 'LANDSCAPER' }, project)).toBe(false);
  ...
  it('does not grant the design-version upload category to Admin', () => {
    ...
    expect(canUploadCategory({ id: 'designer-1', role: 'LANDSCAPER' }, 'DESIGN_VERSION')).toBe(true);
  });

  it('blocks uploads after approval or cancellation', () => {
    const designer = { id: 'designer-1', role: 'LANDSCAPER' as const };
