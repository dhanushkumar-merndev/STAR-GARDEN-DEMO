-- Show every tracked latest call outcome in the sales-member breakdown so the
-- visible outcome total reconciles exactly with Contacted.
...
begin
  ...
  members as (
    select
      ...
      count(lead.id) filter (where outcome.outcome = 'CONNECTED')::integer as connected,
      count(lead.id) filter (where outcome.outcome = 'NO_ANSWER')::integer as no_answer,
      count(lead.id) filter (where outcome.outcome = 'BUSY')::integer as busy,
      count(lead.id) filter (where outcome.outcome = 'SWITCHED_OFF')::integer as switched_off,
