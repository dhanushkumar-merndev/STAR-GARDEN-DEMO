-- Count each lead once, under its latest manual call outcome in the selected
-- range. Repeated clicks/calls must not inflate KPI totals above unique leads.
...
begin
  ...
  activity_counts as (
    select
      (latest.activity_at at time zone 'Asia/Kolkata')::date as day,
      latest.outcome,
      count(*)::integer as count
    from latest_outcome_per_lead latest
    group by 1, latest.outcome
  ),
  totals as (
    select
      outcome.label,
      outcome.sort_order,
      count(latest.lead_id)::integer as count
    from outcomes outcome
    left join latest_outcome_per_lead latest on latest.outcome = outcome.label
...
comment on function public.admin_dashboard_call_outcome_kpis(timestamptz, timestamptz) is
  'Admin-only latest call outcome per unique range lead, with mutually exclusive totals and daily trends.';
